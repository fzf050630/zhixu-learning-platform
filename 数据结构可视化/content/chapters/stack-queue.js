(function (root, factory) {
  ...
  const experiments = [
    ...
    defineExperiment({
      ...
      preset: () => [6, [11, 22, 33], [44, 55, 66, 'dequeue', 66, 77, 'dequeue', 'peek', 'dequeue', 'dequeue', 'dequeue', 'dequeue', 'dequeue', 88]],
      explain: {
        goal: "用取模运算让队尾与队头首尾相接，在固定容量内循环复用空间，并按约定区分队空与队满。",
        inputs: "容量为 capacity 的循环队列（data 数组、front 与 rear 指针），capacity ≥ 2 且保留一个空槽；示例 capacity = 6，演示 14 个入队/出队/读队头操作，含队满与队空边界。",
        steps: [
          "初始化 front = rear = 0：front 指向队头元素，rear 指向下一个可写入的空槽。",
          "入队前判满：若 (rear+1) % capacity == front，说明只剩一个空槽，拒绝入队。",
          "未满则把 x 写入 data[rear]，再令 rear = (rear+1) % capacity；示例中 rear 会从 5 绕回 0。",
          ...
          "当队列再次变空（front 与 rear 重合）后继续出队会报队空，随后仍可重新入队。",
        ],
        keys: [
          "牺牲一个存储单元区分空满：队空 front == rear，队满 (rear+1)%capacity == front。",
          "元素个数 = (rear − front + capacity) % capacity；capacity = 6 时最多存放 5 个元素。",
