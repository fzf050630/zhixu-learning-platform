(function(root,factory){
  ...
  function enhance(e){
    ...
    if(['stack-demo','circular-queue'].includes(e.id)){
      const queue=e.id==='circular-queue',capacity=queue?5:6;
      ...
      change={input:fields([
        ...
        ['operations',queue?'操作：整数入队，dequeue 出队，peek 读队头':'操作：整数入栈，pop 出栈，peek 读栈顶',queue?'44,55,dequeue,55,peek':'39,peek,pop,52','textarea']
    ...
    if(e.id==='sequence-insert')change={input:fields([['values','原数组（无需填空槽）','12,28,41,56'],['index','插入下标（从 0 开始）','2'],['value','插入值','35']],'原数组最多 11 个整数；下标 0 到数组长度。',raw=>{const values=array(raw.values);if(values.length>11)throw new Error('原数组最多 11 项');return{values,index:integer(raw.index,0,values.length),value:integer(raw.value)};}),inputAdapter:input=>[[...input.values,null],input.index,input.value]};
    ...
    if(e.id==='hash-chaining')change={input:fields([['values','关键字','19,14,23,1,68,20'],['size','桶数（1–12）','7']],'支持负数，地址采用 ((key % size) + size) % size。',raw=>({values:array(raw.values),size:integer(raw.size,1,12)})),inputAdapter:input=>[input.values,input.size],code:e.code.map(line=>line.replace('key % H.size','((key % H.size) + H.size) % H.size'))};
  ...
  const crossPreset=()=>({directed:true,nodes:[{id:'A',x:.15,y:.5},{id:'B',x:.5,y:.18},{id:'C',x:.85,y:.5},{id:'D',x:.5,y:.82}],edges:[['A','B',4],['A','C',5],['B','C',-2],['C','D',7],['D','B',6]]});
  ...
  newExperiments.push(defineExperiment({id:'sequence-reverse',title:'顺序表就地逆置',chapter:'linear',tag:'双指针交换',difficulty:'基础',visualizer:'array',generator:extra.sequenceReverse,preset:()=>[[49,38,65,12,27]],input:arrayInput('49, 38, 65, 12, 27'),inputAdapter:values=>[[...values]],explain:{goal:'把顺序表就地逆置，使元素次序完全反转且不使用额外的顺序表。',inputs:'顺序表 L（长度 n），支持按下标随机存取。',steps:['令左指针 i=0 指向表头，右指针 j=n−1 指向表尾。','判断 i 是否小于 j；若不成立说明两端已相遇或交错，逆置完成。','把左端元素 L.data[i] 暂存到临时变量 temp。','把右端元素 L.data[j] 写入左端位置 L.data[i]。','把暂存的 temp 写回右端位置 L.data[j]，完成一对交换。','令 i 自增 1、j 自减 1，向中间收缩，重复判断与交换。','当 i 不小于 j 时停止，顺序表就地逆置完毕。'],keys:['循环条件必须是 i 小于 j；若允许两端相等，中间元素会与自己交换。','只需遍历前一半元素，时间 O(n)，额外空间 O(1)。','必须就地完成，选用额外数组尽管简单但空间不再是 O(1)。'],cost:'时间 O(n) · 空间 O(1)'},code:[
  ...
  newExperiments.push(defineExperiment({id:'sequence-delete-min',title:'顺序表删除最小元素',chapter:'linear',tag:'末位填补',difficulty:'基础',visualizer:'array',generator:extra.sequenceDeleteMin,preset:()=>[[49,38,65,12,27,81,55]],input:arrayInput('49, 38, 65, 12, 27, 81, 55'),inputAdapter:values=>[[...values]],explain:{goal:'删除顺序表中最小的元素，并用引用参数带回它的值。',inputs:'顺序表 L 及引用参数 e；元素可比较大小。',steps:['若表长 n=0，说明是空表，无法删除，直接返回 false。','令 min=0，把第一个元素暂定为最小值。','从 i=1 开始逐个比较 L.data[i] 与 L.data[min]。','若 L.data[i] 更小则更新 min=i，遍历结束后 min 指向最小元素。','把 L.data[min] 赋给引用参数 e，带回被删元素的值。','用表尾元素 L.data[n−1] 填补 min 位置，再把表长减 1。','返回 true；填补后剩余元素的相对次序可能改变。'],keys:['空表必须单独判断并返回失败，否则访问 L.data[0] 会越界。','用末位元素填补被删位置，删除本身只需 O(1)；若要求保持相对次序则需搬移元素，时间 O(n)。','比较用严格小于，多个相同最小值时删除的是最先出现的一个。'],cost:'时间 O(n) · 空间 O(1)'},code:[
  ...
  newExperiments.push(defineExperiment({id:'linked-head-insert',title:'头插法建立单链表',chapter:'linear',tag:'头插法',difficulty:'基础',visualizer:'linked',generator:extra.linkedHeadInsert,preset:()=>[[8,17,26,39]],input:arrayInput('8, 17, 26, 39','1–12 个整数；依次头插后新链表与输入顺序相反。'),inputAdapter:values=>[[...values]],explain:{goal:'用头插法把数组元素依次插入链表头部，建立一个单链表。',inputs:'元素数组 a[0…n−1]；结果链表可不带头结点。',steps:['初始化空链表头指针 head=NULL。','取数组第 i 个元素，申请新结点 p，置 p->data=a[i]。','令新结点 p->next 指向当前表首 head。','令 head=p，新结点成为新的表首。','令 i 自增 1，重复申请结点与头插，直到 n 个元素全部处理完。','返回 head；链表中结点次序与输入顺序相反。'],keys:['头插逆序、尾插保序；两种建表方法的时间都是 O(n)。','每次头插只需修改两个指针，单次 O(1)，无需遍历链表。','head 始终指向最新插入的结点，循环结束后它就是最终表首。'],cost:'时间 O(n) · 空间 O(n)'},code:[
  ...
  newExperiments.push(defineExperiment({id:'deque',title:'双端队列',chapter:'stack-queue',tag:'受限操作',difficulty:'基础',visualizer:'queue',generator:extra.dequeDemo,preset:()=>[5,[11,22,33],['pushFront 44','pushBack 55','pushFront 66','popBack','pushFront 66','popFront','popBack','pushBack 77','popBack','popFront','popBack']],input:fields([['capacity','容量（2–8）','5'],['initial','初始元素（可留空）','11, 22, 33'],['operations','操作序列','pushFront 44, pushBack 55, pushFront 66, popBack, pushFront 66, popFront, popBack, pushBack 77, popBack, popFront, popBack','textarea']],'操作：pushFront x / pushBack x / popFront / popBack；最多 30 项；用 size 计数可装满。',raw=>{const capacity=integer(raw.capacity,2,8);const initial=raw.initial.trim()?array(raw.initial):[];if(initial.length>capacity)throw new Error('初始元素超过容量');const operations=parseOperationList(raw.operations,30);operations.forEach(token=>{const match=/^(pushFront|pushBack|popFront|popBack)(?:\s+([-+]?\d+))?$/.exec(token);if(!match)throw new Error('无法识别的操作：'+token);if(match[1].startsWith('push')&&match[2]===undefined)throw new Error('入队操作需要数值：'+token);if(match[1].startsWith('pop')&&match[2]!==undefined)throw new Error('出队操作不需要数值：'+token);if(match[2]!==undefined)integer(match[2],-999,999);});return{capacity,initial,operations};}),inputAdapter:input=>[input.capacity,input.initial,input.operations],explain:{goal:'用循环数组实现双端队列，两端都可插入与删除，并正确处理队空队满。',inputs:'容量为 capacity 的数组与 size 计数器；front 指向队头，rear 指向队尾的下一次写入位置。',steps:['所有插入操作先判满：仅当 size 等于 capacity 时返回 FULL，拒绝操作。','前插 pushFront：先令 front 环移到前一格 (front−1+capacity) mod capacity，再写入 x，最后 size 加 1。','后插 pushBack：先把 x 写入 data[rear]，再令 rear 环移到 (rear+1) mod capacity，最后 size 加 1。','所有删除操作先判空：仅当 size 等于 0 时返回 EMPTY，拒绝操作。','前删 popFront：先读出 data[front]，再令 front 环移到 (front+1) mod capacity，最后 size 减 1。','后删 popBack：先令 rear 环移到 (rear−1+capacity) mod capacity，再读出 data[rear]，最后 size 减 1。','重复执行操作；size 计数器同时区分队空与队满，capacity 个位置可全部用满。'],keys:['前插与后删都需要先把指针环移再读写；后插与前删则先读写再环移，顺序不能颠倒。','用 size 计数即可装满整个数组，无需像循环队列那样保留一个空槽。','环移公式为 (i−1+capacity) mod capacity 与 (i+1) mod capacity，负号情况要加 capacity 修正。'],cost:'单次操作 O(1) · 空间 O(capacity)'},code:[
  ...
  newExperiments.push(defineExperiment({id:'threaded-tree',title:'中序线索二叉树',chapter:'tree',tag:'线索化',difficulty:'进阶',visualizer:'tree',generator:extra.threadedTree,preset:()=>[{root:'A',nodes:{A:['B','C'],B:['D','E'],C:[null,'F'],D:[null,null],E:[null,null],F:[null,null]}}],input:fields([['root','根节点','A'],['nodes','节点关系（节点 左孩子 右孩子）','A B C\nB D E\nC - F\nD - -\nE - -\nF - -','textarea']],'最多 10 个 A–Z 节点；空孩子写 -；带 * 的结点表示空指针已改造成线索。',treeParse),inputAdapter:tree=>[tree],explain:{goal:'按中序遍历把二叉树中的空指针改造为指向前驱或后继的线索，得到中序线索二叉树。',inputs:'二叉树根结点 root；空指针用 NULL 表示，线索标记为 ltag/rtag。',steps:['从根出发按中序顺序线索化，先递归处理当前结点 p 的左子树。','回到 p 后，若 p 的左指针为空，则令它指向<b>中序前驱</b> pre，并置 ltag=1。','若前驱 pre 非空且其右指针为空，则令 pre 的右指针指向 p（后继），并置 rtag=1。','令 pre=p，使 pre 始终记录刚访问过的结点，即下一个结点的前驱。','递归线索化 p 的右子树，重复上述空指针改造。','遍历结束时所有空指针都变成线索，非空孩子指针保持原样（tag 为 0）。'],keys:['只改造空指针：左空指针指前驱、右空指针指后继，非空孩子指针绝不能被覆盖。','线索化必须按中序顺序进行，pre 初值必须为 NULL，否则第一条左线索会写错。','判断真孩子与线索要看 ltag/rtag，不能直接沿指针继续遍历，否则会沿线索绕成环。'],cost:'时间 O(n) · 空间 O(h)（递归栈）'},code:[
  ...
  newExperiments.push(defineExperiment({id:'heap-insert',title:'大根堆插入与上调',chapter:'sort',tag:'向上调整',difficulty:'基础',visualizer:'array',generator:extra.heapInsert,preset:()=>[[95,53,78,17,45,65,12],82],input:fields([['values','初始大根堆（层序）','95, 53, 78, 17, 45, 65, 12'],['value','插入值','82']],'层序输入且必须已是大根堆（父结点不小于孩子）；插入值 −999 到 999。',raw=>{const values=array(raw.values);if(!maxHeapCheck(values))throw new Error('输入序列不是合法的大根堆（父结点须不小于孩子）');return{values,value:integer(raw.value)};}),inputAdapter:input=>[input.values,input.value],explain:{goal:'向大根堆中插入一个新关键字，通过向上调整恢复大根堆性质。',inputs:'已满足大根堆性质的层序数组 A[0…n−1]，待插入值 key。',steps:['把新关键字追加到数组末尾 A[n]，令 i=n 并令 n 自增 1，保持完全二叉树形态。','若 i 大于 0，计算双亲下标 parent=⌊(i−1)/2⌋。','比较 A[i] 与 A[parent]：若不大于双亲，说明已满足大根堆，立即停止。','若大于双亲，则交换 A[i] 与 A[parent]，并令 i=parent 继续向上比较。','重复比较与交换，新元素沿祖先路径逐层上浮。','当到达根（i=0）或不再大于双亲时结束，插入后仍是大根堆。'],keys:['插入只能在表尾追加，保证堆仍是一棵完全二叉树，不会破坏结构性质。','新元素只可能向上移动，调整代价与树高同阶，即 O(log n)。','判定停止用「不大于双亲」而非「小于」，相等时不交换可减少移动次数。'],cost:'时间 O(log n) · 空间 O(1)'},code:[
  ...
  newExperiments.push(defineExperiment({id:'hash-double',title:'双散列探测',chapter:'search',tag:'双散列',difficulty:'进阶',visualizer:'hash',generator:extra.hashDouble,preset:()=>[[19,14,23,1,68,20,56],11,5],input:fields([['keys','关键字','19, 14, 23, 1, 68, 20, 56'],['size','表长（3–12）','11'],['prime','第二散列模 prime（2 ≤ prime < 表长）','5']],'H1=key mod 表长；H2=prime − key mod prime 决定探测步长；对比线性探测的堆积。',raw=>{const size=integer(raw.size,3,12);const prime=integer(raw.prime,2,12);if(prime>=size)throw new Error('prime 必须小于表长');return{keys:array(raw.keys),size,prime};}),inputAdapter:input=>[input.keys,input.size,input.prime],explain:{goal:'用双散列探测解决冲突，使探测序列随关键字变化，减少堆积。',inputs:'表长 m（3–12）、第二散列模数 prime（不小于 2 且小于表长）与关键字序列。',steps:['计算第一散列地址 h1=key mod m。','计算第二散列值 h2=prime−(key mod prime)，它作为探测步长，恒大于 0。','从 d=0 开始，按公式 pos=(h1+d·h2) mod m 计算第 d 次探测地址。','若 table[pos] 为空位，存入 key、元素个数加 1，插入成功返回。','若位置已被占用，令 d 增 1，按同一公式跳到下一个探测位置。','若探测 m 次仍未找到空位，说明表已满，插入失败。'],keys:['双散列探测公式为 H_i=(H1(key)+i·H2(key)) mod m，步长随关键字变化，可缓解堆积。','第二散列必须保证非零且与表长互质；prime 取素数、表长取素数时探测序列更易遍历全表。','当步长与 m 不互质时探测序列可能提前循环，导致有空洞却插不进去。','删除同样需要删除标记，否则探测链断裂会影响后续查找。'],cost:'平均 O(1) · 最坏 O(n) · 空间 O(m)'},code:[
    'bool Insert(HashTable &H, Key key) {',
    '  int h1 = key % H.size;',
