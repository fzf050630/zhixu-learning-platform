...
最新交付（2026-09-21）：红黑树、B 树、B+ 树补全**删除**操作，实验更名为「插入、删除与查找」；AVL 由四类预制旋转升级为**通用插入 + 删除**引擎。红黑树按 CLRS 演示「双黑」修复四情形（渲染器显式画出 NIL 空槽与双环「双黑」标记），B 树 t=2 与 B+ 树演示借位、合并与根降高，B+ 树删除后重算父索引并重接叶链 `next`。输入新增「删除序列」（可留空，不存在的键会演示跳过）。新增 `tests/tree-invariants.cjs`、`tests/advanced-tree-delete.test.js`、`tests/avl-tree.test.js`；Node 91/91 通过。实验总数仍为 54。
...
- 查找：折半查找、BST 插入、BST 查找、BST 删除、AVL 树插入与删除、红黑树插入/删除/查找、B 树插入/删除/查找、B+ 树插入/删除/查找、散列表冲突处理、线性探测、双散列、KMP、KMP nextval、BF 匹配
