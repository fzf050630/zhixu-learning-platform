...
(function (global) {
  ...
  /** 标准外壳，返回 { cv, ctrl, out, scene, host } */
  ...
  function slider(parent, o) {
    ...
    return {
      input: inp,
      set(v) { inp.value = v; val.textContent = o.fmt ? o.fmt(v) : v; }
    };
  }
  ...
  function number(parent, o) {
    ...
    _onlyNumber(inp, o);
    g.appendChild(lab); g.appendChild(inp);
    if (o.suffix) g.appendChild(el('span', 'unit', o.suffix));
    parent.appendChild(g);
    return {
      input: inp,
      get value() { return Number(inp.value); },
      set(v) { inp.value = v; },
      onChange(fn) { inp.addEventListener('input', () => fn(Number(inp.value))); return this; }
    };
  }
  ...
  function text(parent, o) {
    ...
    inp.setAttribute('aria-label', o.label);
    inp.style.width = (o.width || 200) + 'px';
    g.appendChild(lab); g.appendChild(inp);
    parent.appendChild(g);
    return { input: inp, get value() { return inp.value; }, set(v) { inp.value = v; } };
  }

  function _onlyNumber(inp, o) {
  ...
  /** 播放控制条：返回 { play, pause, prev, next, at, index, total, setSpeed } */
  function transport(parent, o) {
    const wrap = el('div', 'transport-viz');
    const on = o.onChange || (() => { });
    let i = 0, timer = null, speed = o.speed || 900;
    ...
    function render() { bIdx.textContent = (i + 1) + ' / ' + total; }
    function go(k) {
  ...
  global.UI = {
    el, shell, shellPlot, slider, seg, legend, readout, note,
    number, text, button, transport, f2, f3, f4, pct, C
  };
