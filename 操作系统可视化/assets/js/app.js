...
(function (global) {
  ...
  function renderOverview() {
    ...
    h1.className = 'blk';
    h1.textContent = SYL.outlineTitle || '教材结构';
    frag.appendChild(h1);
    ...
    if (es) {
      ...
      esEl.innerHTML =
        `<div class="tbl-wrap"><table class="tbl"><thead><tr><th>科目</th><th class="center">分值</th></tr></thead><tbody>` +
        es.parts.map(p => `<tr><td>${p.name}</td><td class="center">${p.score}</td></tr>`).join('') +
        `</tbody></table></div>` +
        `<div class="tbl-wrap"><table class="tbl"><thead><tr><th>题型</th><th class="center">题数</th><th class="center">分值</th></tr></thead><tbody>` +
    ...
    note.innerHTML = '<span style="color:var(--ink-3);font-size:13.5px">下表对照王道教材章节列出主要考点与对应页面锚点，点击可直达，用于机械核对「不漏知识点」。</span>';
